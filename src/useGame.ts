...
export function useGame() {
  ...
  const generateObstacles = useCallback((levelDef: LevelDef): Point[] => {
    ...
    const count = Math.floor((levelDef.width * levelDef.height) * 0.05); // 5% density
    ...
    for (let i = 0; i < count; i++) {
      ...
      while (!isValid) {
        ...
        const inCenter = x > 5 && x < 15 && y > 5 && y < 10;
        const alreadyExists = obstacles.some(o => o.x === x && o.y === y);
        if (!inCenter && !alreadyExists) isValid = true;
      }
      if (x !== undefined && y !== undefined) obstacles.push({ x, y });
    }
    return obstacles;
  }, []);
  ...
  const moveSnake = useCallback(() => {
    setGameState(prev => {
      ...
      if (nextLevelDef && newTotalPoints >= nextLevelDef.unlockPoints && !unlockedLevels.includes(nextLevelDef.id)) {
        isLevelUp = true;
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      const speedMs = 1000 / currentLevelDef.speed;
